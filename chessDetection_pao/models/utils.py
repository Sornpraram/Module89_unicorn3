...
plt_epoch = []
plt_loss = []
def save_loss(loss, epoch, model):
...
def Load_Yolo_model():
    ...
    if YOLO_FRAMEWORK == "tf": # TensorFlow detection
        ...
        if YOLO_CUSTOM_WEIGHTS == False:
            ...
            yolo = Create_Yolo(input_size=INPUT_RESOLUTION, CLASSES=YOLO_COCO_CLASSES)
...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(0,255,0), rectangle_colors='', tracking=False):   
    ...
    for i, bbox in enumerate(bboxes):
        ...
        fontScale = 0.55 * bbox_thick
        (x1, y1), (x2, y2) = (coor[0], coor[1]), (coor[2], coor[3])

        # put object rectangle
        cv2.rectangle(image, (x1, y1), (x2, y2), bbox_color, bbox_thick*1)
        ...
        if show_label:
            ...
            cv2.putText(image, label, (x1, y1-4), cv2.FONT_HERSHEY_COMPLEX_SMALL,
                        fontScale, Text_colors, 1, lineType=cv2.LINE_AA)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.6, iou_threshold=0.45, rectangle_colors='', crop=False):
    original_image      = cv2.imread(image_path)
    if crop:
        original_image      = original_image[0:1080,320:1590]
...
def detect_video(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.3, iou_threshold=0.3, rectangle_colors='', crop=False):
    times, times_2 = [], []
    vid = cv2.VideoCapture(video_path)
    ...
    while True:
        _, img = vid.read()
        if crop==True:
            # img      = img[0:1080,320:1590] #image[0:1080,420:1520]
            img      = img[0:1080,420:1520]
            # img      = img[236:844,656:1264]
        ...
        image = cv2.putText(image, "Time: {:.1f}FPS".format(fps), (0, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
        # CreateXMLfile("XML_Detections", str(int(time.time())), original_image, bboxes, read_class_names(CLASSES))
...
# detect from webcam
# default: 480p (640 x 480)
def detect_realtime(Yolo, output_path, input_size=416, show=False, CLASSES=TRAIN_CLASSES, score_threshold=0.5, iou_threshold=0.45, rectangle_colors='', crop=False):
    times = []
    vid = cv2.VideoCapture(0)
