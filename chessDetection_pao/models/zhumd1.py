...
from models.utils import read_class_names
...
def darknet19_tiny(input_data):
    input_data = convolutional(input_data, (3, 3, 3, 16), downsample=True)
    input_data = convolutional(input_data, (3, 3, 16, 32), downsample=True)
...
def YOLOv3_tiny(input_layer, NUM_CLASS):
    ...
    conv = convolutional(conv, (1, 1, 1024, 256)) # conv7
    conv_lobj_branch = convolutional(conv, (3, 3, 256, 512)) # conv8
    
    # conv_lbbox is used to predict large-sized objects , Shape = [None, 26, 26, 255]
    conv_lbbox = convolutional(conv_lobj_branch, (1, 1, 512, 3*(NUM_CLASS + 5)), activate=False, bn=False) # conv9

    # conv = convolutional(conv, (1, 1, 256, 128)) # conv10
    ...
    # conv = conv + route_1
    conv_mobj_branch = convolutional(conv, (3, 3, 128, 256)) # conv11
...
def Create_Model(input_size=416, channels=3, training=False, CLASSES=YOLO_COCO_CLASSES):
    ...
    # Yolo.summary()
    # keras.utils.plot_model(Yolo, to_file='modelv3.png', show_shapes=False)
    return Yolo
